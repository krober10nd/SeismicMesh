...
![The mesh creation time and resulting mesh quality for four bencmark problems studied. Two analytical problems (disk and a ball) and two non-anlaytical problems with sizing functions defined via interpolants (BP2004 and EAGE). All quantities are annotated next to points in the color that matches the generator used. For the cell quality panels a straight line indicates the minimum cell quality and the dot indicates the average cell quality. \label{fig:benchmark}](Benchmarks.jpg)
